...
| Subdivision | `a_subdivision[]=<value>` | NOT `subdivision`. Value is the exact subdivision-name string (e.g. `Anthem at Merrill Ranch`), url-encoded with `+` for spaces — same pattern as `a_propSubType`, no separate ID/code. No `ccz`-style envelope key needed. Confirmed 2026-09-22 by selecting "Anthem at Merrill Ranch" in the live advanced-search UI at search.arizonabuyandsell.com and reading the resulting results URL: `...&a_subdivision%5B%5D=Anthem+at+Merrill+Ranch`. The same page's `a_subdivision[]` `<select multiple>` supplied the full 300+ option list used for the widget's picker. |
